...
public class AdminDao {
	...
	public List<Aluno> listaAlunosPorDisciplinaStatus(Disciplina d, int codigoStatus, int turno) throws SQLException{
		List<Aluno> lista = new ArrayList<Aluno>();
		String sql = "select distinct aluno.ra, aluno.nome from termo inner join aluno on aluno.ra = termo.ra_aluno inner join disciplina on termo.codigo_disciplina = disciplina.codigo inner join paises on paises.codigo = termo.codigo_pais inner join estado on estado.codigo = termo.codigo_status where termo.codigo_disciplina = ? AND termo.codigo_status = ? ";
		if (turno != 0){
			sql = sql + " AND aluno.turno = ?";
		}
		sql = sql + " ORDER BY aluno.nome";
		PreparedStatement ps = c.prepareStatement(sql);
