...
public class AdminDao {
	...
	public List<Termo> listaAssuntosPorAlunoDisciplinaStatus(Aluno a, Disciplina d, int codigoStatus) throws SQLException{
		...
		String sql = "select aluno.ra, aluno.nome, termo.codigo_disciplina, disciplina.sigla as disciplina, termo.codigo_pais, paises.nome as pais, termo.assunto,	termo.termo, termo.codigo_status, estado.estado from termo inner join aluno on aluno.ra = termo.ra_aluno inner join disciplina on termo.codigo_disciplina = disciplina.codigo inner join paises on paises.codigo = termo.codigo_pais inner join estado on estado.codigo = termo.codigo_status where termo.codigo_disciplina = ? AND aluno.ra = ? AND termo.codigo_status = ? ORDER BY paises.nome";
		PreparedStatement ps = c.prepareStatement(sql);
	...
	public List<Termo> listaTermosPorAlunoDisciplinaStatus(Aluno a, Disciplina d, int codigoStatus) throws SQLException{
		List<Termo> lista = new ArrayList<Termo>();
		String sql = "select aluno.ra, aluno.nome, termo.termo, termo.codigo_disciplina, disciplina.sigla as disciplina, termo.codigo_pais, paises.nome as pais, termo.assunto,	termo.termo, termo.codigo_status, estado.estado from termo inner join aluno on aluno.ra = termo.ra_aluno inner join disciplina on termo.codigo_disciplina = disciplina.codigo inner join paises on paises.codigo = termo.codigo_pais inner join estado on estado.codigo = termo.codigo_status where termo.codigo_disciplina = ? AND aluno.ra = ? AND termo.codigo_status = ? ORDER BY paises.nome";
		PreparedStatement ps = c.prepareStatement(sql);
		ps.setInt(1, codigoStatus);
		ps.setString(2, a.getRa());
		ps.setInt(3, d.getCodigo());
